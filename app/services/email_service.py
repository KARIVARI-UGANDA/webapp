...
def _build_receipt_pdf(
    ...
    sub_style = ParagraphStyle(
        "sub",
        fontSize=11,
        textColor=colors.HexColor("#93c287"),
        fontName="Helvetica",
        alignment=TA_CENTER,
        leading=16,
    )
    label_style = ParagraphStyle(
        "label", fontSize=11, textColor=grey, fontName="Helvetica", alignment=TA_LEFT
    )
    ...
    detail_rows = [
        ...
        row("Amount Paid  (USD / EUR)", f"${usd:,.2f}  /  €{eur:,.2f}", big=True),
    ]
    ...
    story = [
        ...
        Spacer(1, 3 * mm),
        Paragraph(
            "© 2026 Kari Vari Uganda · Kampala &amp; Entebbe, Uganda", footer_style
        ),
    ]
...
def send_booking_confirmation(
    ...
    _send(
        to_email, f"Rental Confirmed — {vehicle_name} · Kari Vari Uganda", _wrap(body)
    )
...
def send_invoice(
    ...
    rows = [
        ...
        (
            "Transaction ID",
            f'<span style="font-size:12px;color:#555;">{gateway_ref}</span>',
        ),
        ("Amount Paid", _ugx_to_display(amount_ugx)),
    ]
