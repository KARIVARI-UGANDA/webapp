...
class TestCreateBooking:
    def test_customer_can_create_booking(self, client, customer, verified_vehicle):
        ...
        r = client.post("/api/bookings/", headers=headers, json={
            **BOOKING_PAYLOAD, "vehicle_id": verified_vehicle
        })
    ...
    def test_booking_nonexistent_vehicle(self, client, customer):
        headers, _ = customer
        r = client.post("/api/bookings/", headers=headers, json={
            **BOOKING_PAYLOAD, "vehicle_id": "99999999"
        })
...
class TestListBookings:
    ...
    def test_customer_cannot_see_others_bookings(self, client, customer, verified_vehicle):
        from tests.conftest import _register, _auth_headers
        _register(client, "cust2@test.com", "password123", "Cust Two", phone="+256705000001")
...
class TestOwnerBookings:
    def test_owner_sees_bookings_for_their_vehicles(self, client, owner, customer, verified_vehicle):
        customer_headers, _ = customer
        owner_headers, _ = owner
        client.post("/api/bookings/", headers=customer_headers, json={
            **BOOKING_PAYLOAD, "vehicle_id": verified_vehicle
        })
...
class TestCancelBooking:
    def test_customer_can_cancel_pending_booking(self, client, customer, verified_vehicle):
        headers, _ = customer
        create = client.post("/api/bookings/", headers=headers, json={
            **BOOKING_PAYLOAD, "vehicle_id": verified_vehicle
        })
