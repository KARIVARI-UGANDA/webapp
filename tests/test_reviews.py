...
class TestCreateReview:
    def _make_confirmed_booking(self, client, customer_headers, owner_headers, admin_headers):
        ...
        r = client.post("/api/vehicles/", headers=owner_headers, json={
            "make": "Toyota", "model": "Prado", "year": 2021, "color": "Black",
            "registration_plate": "UAC 001A", "vehicle_type": "safari",
            "transmission": "automatic", "fuel_type": "diesel",
            "passenger_capacity": 5, "has_ac": True, "is_4wd": True,
            "base_daily_rate_ugx": 600000, "service_area": "Kampala",
        })
        vid = r.json()["id"]
        ...
        # Book it
        today = date.today()
        booking = client.post("/api/bookings/", headers=customer_headers, json={
            "vehicle_id": vid,
            "pickup_date": (today + timedelta(days=3)).isoformat(),
            "return_date": (today + timedelta(days=5)).isoformat(),
            "pickup_location": "Kampala", "dropoff_location": "Jinja",
            "passenger_count": 2,
        })
        return booking.json().get("booking_id") or booking.json().get("id"), vid

    def test_cannot_review_pending_payment_booking(self, client, customer, owner, admin):
        ...
        r = client.post("/api/reviews/", headers=customer_headers, json={
    ...
    def test_unauthenticated_cannot_review(self, client):
        r = client.post("/api/reviews/", json={
            "booking_id": "some-id", "reviewee_id": "some-id",
            "review_target": "trip", "overall_rating": 5,
        })
    ...
    def test_invalid_rating_rejected(self, client, customer):
        ...
        r = client.post("/api/reviews/", headers=headers, json={
            "booking_id": "x", "reviewee_id": "y",
            "review_target": "trip", "overall_rating": 6,
        })
