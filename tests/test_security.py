...
class TestTokenSecurity:
    ...
    def test_customer_token_cannot_create_vehicle(self, client, customer):
        headers, _ = customer
        r = client.post("/api/vehicles/", headers=headers, json={
            "make": "Ford", "model": "Ranger", "year": 2022, "color": "Blue",
            "registration_plate": "UAD 001A", "vehicle_type": "suv",
            "transmission": "manual", "fuel_type": "diesel",
            "passenger_capacity": 4, "base_daily_rate_ugx": 500000,
        })
        assert r.status_code == 403
    ...
    def test_suspended_user_cannot_login(self, client, admin):
        ...
        _register(client, "suspend@test.com", "password123", "Suspend Me", phone="+256706000001")
...
class TestRoleIsolation:
    def test_owner_cannot_book_own_vehicle(self, client, owner, verified_vehicle):
        ...
        r = client.post("/api/bookings/", headers=headers, json={
            ...
            "pickup_location": "Kampala", "dropoff_location": "Entebbe",
            "passenger_count": 1,
        })
    ...
    def test_customer_cannot_approve_vehicles(self, client, customer, owner):
        ...
        create = client.post("/api/vehicles/", headers=owner_headers, json={
            "make": "Honda", "model": "CR-V", "year": 2020, "color": "Grey",
            "registration_plate": "UAD 100A", "vehicle_type": "suv",
            "transmission": "automatic", "fuel_type": "petrol",
            "passenger_capacity": 5, "base_daily_rate_ugx": 450000,
        })
        vid = create.json()["id"]
        r = client.patch(f"/api/admin/verifications/{vid}/approve", headers=customer_headers)
        assert r.status_code == 403
